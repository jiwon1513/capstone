...
cap = cv2.VideoCapture("C:/jiwon/please/seg1.mp4")
pt = 0
fps = 50
...
while (cap.isOpened()):
    ...
    if(ret is True)and(ct>1./fps) :
        ...
        #cv2.imshow('can',can)
        contours = fr.findvertex(image) + fr.findvertex1(image)      
       
      
        mask = fr.region_of_interest(can,contours)
        cv2.imshow('s',mask)
